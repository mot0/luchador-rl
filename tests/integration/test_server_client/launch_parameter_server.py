...
def _build_model():
    ...
    model_def = luchador.nn.get_model_config(
        'vanilla_dqn', n_actions=4, input_shape=input_shape)

    dql = luchador_rl.agent.rl.q_learning.DeepQLearning(
        q_learning_config={
            'discount_rate': 0.99,
            'min_reward': -1.0,
            'max_reward': 1.0,
...
def _main():
    ...
    dql = _build_model()
    app = luchador_rl.agent.remote.create_parameter_server_app(dql.session)
